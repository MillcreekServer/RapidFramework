...
public class ELOUtil {
	 ...
	 * @param scoreCandidate1 the game result. 1 if win, 0.5 for draw, and 0 for lose
	 * @param scoreCandidate2 the game result. 1 if win, 0.5 for draw, and 0 for lose
	...
	public static ELOData caculateNewElo(ELOData data, int scoreCandidate1, int scoreCandidate2, int K_factor) {
		...
		return new ELOData(
				data.candidate1 + K_factor * (scoreCandidate1 - expected1),
				data.candidate2 + K_factor * (scoreCandidate2 - expected2));
	}
	
	/**
	 * Get floor value of given value up to the given factor.
	 * The factor should be multiple of 10 which represents
	 * up to that decimal points. 10 will floor up to one decimal places.
	 * @param value
	 * @param factor
	 * @return
	 */
