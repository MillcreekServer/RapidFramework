...
/**
 * <p>Simple permission system that will be internally used.
 * Permissions here are not related to the Bukkit API at all, so permissions
 * used here are not reflected from/to Bukkit API's permission system.</p>
 * 
 * <p>Also, "simple" means that the permission string doesn't have such thing
 * like parent and children (like permission.child.child in Bukkit API) but
 * have a plain string that either the permission holder has the permission
 * string or not. Don't expect too much from this manager.</p>
 * 
 * <p>Though, it can detect if the parent group of the permission holder
 * has such permission without the caller explicitly checking on every parents.
 * For example, if player1 is under group1, group1 has permission xyz, and you
 * check if the player1 has permission xyz, it will search through the group
 * hierarchy to see if this player has the required permission. The lower level
 * permission has higher priority when there are duplicating permissions in the
 * hierarchy.</p>
 * 
 * @author wysohn
 *
...
public class ManagerCustomPermission extends ManagerElementCaching<PluginBase, UUID, Permissions> {
	...
	/**
	 * Check if the target has at least one of the permission in given permissions. If
	 * parent and current target has conflicting permission settings, the current
	 * one will be used. (e.g. if parent group of player1 has no permission but player1 has
	 * permission X, then player1 is considered to have the permission X)
	 * 
	 * <p> To use the hierarchical permission system, PermissionHolderProvider must be
	 * registered with {@link #registerProvider(PermissionHolderProvider)}
	 * if you have custom child class of PermissionHolder </p>
	 * 
	...
	/**
	 * Remove the permStr from the target. Nothing happens if permission doesn't exist
	 * @param target
	 * @param permStrs
	 */
