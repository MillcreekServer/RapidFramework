...
public class JsonApiAPI extends APISupport {
    ...
    private JsonMsg toJsonMessage(JsonApiAPI.Message[] messages) {
        JsonMsg jsonMessages = new JsonMsg();
        for (int i = 0; i < messages.length; i++) {
            Message message = messages[i];

            jsonMessages.append(message.string);
            if (message.click_OpenFile != null) {
                jsonMessages.clickEvent(JsonClickEvent.openFile(message.click_OpenFile));
            }
            if (message.click_OpenUrl != null) {
                jsonMessages.clickEvent(JsonClickEvent.openUrl(message.click_OpenUrl));
            }
            if (message.click_RunCommand != null) {
                jsonMessages.clickEvent(JsonClickEvent.runCommand(message.click_RunCommand));
            }
            if (message.click_SuggestCommand != null) {
                jsonMessages.clickEvent(JsonClickEvent.suggestCommand(message.click_SuggestCommand));
            }
            if (message.hover_ShowText != null) {
                jsonMessages.hoverEvent(JsonHoverEvent.showText(message.hover_ShowText));
            }
            if (message.hover_ShowAchievement != null) {
                jsonMessages.hoverEvent(JsonHoverEvent.showAchievement(message.hover_ShowAchievement));
            }
            if (message.hover_ShowItem != null) {
                jsonMessages.hoverEvent(JsonHoverEvent.showItem(message.hover_ShowItem));
            }
        }
    ...
    public static class MessageBuilder {
        ...
        /**
         * This changes the current message to the specified and append it to
         * the array. So once you call this method, you lose the ability to
         * build the previous message.
         * 
         * @param str
         *            string to append. Putting null in str does nothing.
