...
public class ManagerStructure<PB extends PluginBase> extends ManagerElementCaching<PB, SimpleLocation, Structure>
    ...
    public ManagerStructure(PB base, int loadPriority, String structureClassParentPath) {
        ...
        if (structureClassParentPath == null)
            throw new RuntimeException(
                    "structureClassParentPath must be set in order for ManagerStructure " + "to work properly!");
    ...
    public void onInteract(PlayerInteractEvent e) {
        ...
        if (e.getAction() == Action.LEFT_CLICK_BLOCK) {
             ...
             * else if (e.getPlayer().isSneaking() && structure instanceof
             * ClickableLeftShift) { ((ClickableLeftShift) structure).onClick(base,
             * e.getAction(), e.getPlayer(), structurePermissionFilter); }
             */
            ...
            if (structure instanceof ClickableRight) {
                ...
                ((ClickableRightShift) structure).onClick(base, e.getAction(), e.getPlayer(),
                        structurePermissionFilter);
            }
    ...
    public void onClickWithStructureItem(PlayerInteractEvent e) {
        ...
        if (this.get(sloc) != null) {
            base.sendMessage(player, DefaultLanguages.Manager_Structure_AlreadyThere);
            return;
        }
        ...
        if (clicked.getType() != requiredType) {
            ...
            base.sendMessage(player, DefaultLanguages.Manager_Structure_NotAValidBlock);
            return;
        }
        ...
        try {
            ...
            base.getLogger().warning(
                    "A player is trying to set up structure " + title + ", but no such structure class found.");
            return;
        }
        ...
        if (InventoryUtil.consumeOneItemInHand(e.getPlayer())) {
            try {
                ...
                base.getLogger().severe(
                        player.getName() + " spent a structure item for " + title + " but structure couldn't be set");
            }
        } else {
    ...
    public boolean startStructureEdit(Player player, SimpleLocation sloc) {
        ...
        pemanager.startEdit(player, sloc.toString(), structure.infoToMap(player, uuidToNameParse),
                new ConversationAbandonedListener() {

                    @Override
                    public void conversationAbandoned(ConversationAbandonedEvent arg0) {
                        Map<Language, Object> props = (Map<Language, Object>) arg0.getContext()
