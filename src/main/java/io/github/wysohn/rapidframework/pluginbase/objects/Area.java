...
public class Area implements Iterable<SimpleLocation> {
    ...
    /**
     * Check wether the area1 and area2 conflicts. Conflict means at least a block
     * in the area is duplicating with another block in the second area.
     *
     * @param area1 area1
     * @param area2 area2
    ...
    public static Area fromChunk(SimpleChunkLocation scloc) {
        SimpleLocation sloc1 = new SimpleLocation(scloc.world, scloc.i, 0, scloc.j);
        SimpleLocation sloc2 = new SimpleLocation(scloc.world, scloc.i + 15, 255, scloc.j + 15);
        return new Area(sloc1, sloc2);
    }
     ...
     * @param sloc
     * @return
     * @deprecated method name can be confusing. Same as using
     * {@link #isContainingLocation(SimpleLocation)}
     */
    @Deprecated
    ...
    /**
     * Get the number of blocks in the area. This calculates mathematically each
     * time, so it's better to call it only once unless you need some dynamic
     * values.
     *
