...
/**
 * Simple class designed to do some repetitive tasks 'lazily.' This 'lazy' behavior is
 * useful when several repetitive tasks that doesn't have to run every time. For example,
 * to keep the data in memory safe, we write them into the permanent storage (like HDD), 
 * however, the data in memory often change really quickly, and writing the data to 
 * permanent storage every single time when the data in memory gets changed is quite a waste of
 * resources. If there was 100 changes in the data, it's better to wait for certain amount
 * of time before actually writing data to the storage to get up to date memory data instead
 * of saving them 100 times individually.
 * overheads.
 * @author wysohn
 *
 */
public class ManagerLazyTask extends PluginManager<PluginBase>{
	...
	/**
	 * schedule the lazy task. If task fails and throws exception, it
	 * will remove the tasks from the queue.
