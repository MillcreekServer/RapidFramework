...
public class ManagerVolatileTask extends PluginManager<PluginBase> implements Listener {
    ...
    public void onBlockMove(PlayerBlockLocationEvent e) {
        ...
        if (cancelAll(tasks, e.getPlayer().getUniqueId())) {
            base.sendMessage(e.getPlayer(), DefaultLanguages.Manager_VolatileTask_CanceledCauseMoved);
        }
    }
     ...
     *
     * @param taskKey    this key ensures that no other task can be scheduled with
     *                   same key until scheduled one is finished.
     * @param player
     * @param run
     * @param delayTicks
